...
# - Import statements reordered/reformatted.
import re
...
RE_TIME_2 = re.compile(r"([0-1]?[0-9]|2[0-3])点" r"([0-5][0-9])分" r"(([0-5][0-9])秒)?")
RE_TIME_3 = re.compile(r"([0-5]?[0-9])分" r"(([0-5][0-9])秒)?")
...
def replace_time_nohour(match) -> str:
    """
    Args:
        match (re.Match)
    Returns:
...
def replace_time(match) -> str:
    ...
    if minute.lstrip("0"):
        if int(minute) == 30 and not second:
            result += "半"
        else:
...
RE_DATE = re.compile(
    ...
    r"(\d{4})年" r"((0?[1-9]|1[0-2])月)?" r"(((0?[1-9])|((1|2)[0-9])|30|31)([日号]))?"
)
...
# 用 / 或者 - 分隔的 YY/MM/DD 或者 YY-MM-DD 日期 或者 YY_MM_DD
RE_DATE2 = re.compile(r"(\d{4})([-_ /.])(0?[1-9]|1[012])\2([12][0-9]|3[01]|0?[1-9])")
